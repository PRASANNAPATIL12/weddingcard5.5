  ...
  const loadWeddingData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Loaded wedding data:', data);
        setWeddingData(data);
        // Generate shareable URL using wedding data
        generateWeddingUrl(data);
      } else {
        console.log('No existing wedding data found, keeping defaults');
        // User will have default template data to edit
      }
    } catch (error) {
      console.error('Error loading wedding data:', error);
      console.log('Using default template data for editing');
    }
  };

  const generateWeddingUrl = (weddingData) => {
    // Use shareable_id if available, otherwise fallback to regular id
    const shareableId = weddingData?.shareable_id || weddingData?.id;
    if (shareableId) {
      // Generate shareable URL using /share/ route
